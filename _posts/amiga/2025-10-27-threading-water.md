...
This fiddling with registers is a bit hackish. The `DEBUG` define does try to tell the compiler to not overwrite the value in the register, but it seems to not always comply with this (I'm not saying it's random, but I guess the compiler can optimize these things out -- I might come up with better ways to solve this). I've noticed that if you use the lower registers like `r0` they can contain other values when viewed with `XRegs`. It also depends on what you actually do in the function -- if you use more variables you'll have fewer registers to play with.
...
In this example we start 8 threads on core 0, and 8 threads on core 1.
...
Now, here we see all the 8 threads having their individual values. And this on core 0. If we want to see what core 1 is up to, we can use the `-j` option to choose which core to see.
